...
async function run() {
  ...
  db.run(`
    ...
    CREATE TABLE IF NOT EXISTS thuoc (id INTEGER PRIMARY KEY AUTOINCREMENT, id_bac_si INTEGER NOT NULL, FOREIGN KEY (id_bac_si) REFERENCES bac_si(id) ON UPDATE CASCADE ON DELETE RESTRICT);
  ...
  try {
    ...
    console.log("Success with FK ON!");
  } catch (e) {
